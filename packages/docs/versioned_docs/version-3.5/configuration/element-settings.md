...
- **Required:** Yes
- **Description:** Function that executes when interaction is predicted or occurs. This is where your prefetching logic goes. The callback receives the [`ForesightElementData`](../getting-started/typescript.md#foresightelementdata) of the element that triggered it, which is especially useful when registering multiple elements with a shared callback.
- **Note:** If you await your prefetch logic the `callbackCompleted` [event](../events.md#callbackcompleted) will show you how long your prefetch took to run.
...
- **Required:** No
- **Default:** `{}`
- **Description:** Stores additional information about the registered element. Visible in all element-related [events](../events.md) and in the [devtools](../debugging/devtools.md).
