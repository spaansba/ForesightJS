...
- **Type:** `boolean`
- **Default:** `false`
- **Description:** Logs basic information about the `ForesightManager` and its handlers that is not available through [events](../events.md). Mostly used by the maintainers of `ForesightJS` to debug the manager, but might be useful for implementers aswell.
...
Visual development tools are available as a separate package. See the [development tools documentation](../debugging/devtools.md) for details on installing and configuring the `js.foresight-devtools` package.
:::
