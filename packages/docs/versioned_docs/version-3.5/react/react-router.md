...
Below is an example of creating an wrapper around the React Router `Link` component that prefetches with ForesightJS. On mobile devices ForesightJS uses the configured [`touchDeviceStrategy`](../configuration/global-settings.md#touch-device-settings-v330). This implementation uses the `useForesight` react hook which can be found [here](./hook.md).
