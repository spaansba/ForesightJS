...
The `useForesight` composable provides a Vue-native way to integrate ForesightJS into your Vue components. Unlike the [`v-foresight` directive](./directive.md), the composable gives you direct access to [`registerResults`](../configuration/element-settings.md#registration-return-value) and the element's [`templateRef`](https://vuejs.org/guide/essentials/template-refs), but requires more setup code.
...
- [`registerResults`](../configuration/element-settings.md#registration-return-value) - Registration details like `isRegistered`

### Basic Usage

This is the most basic way to use the composable and is basically the same as using the [`v-foresight` directive](./directive.md).
