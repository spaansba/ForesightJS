...
The `v-foresight` directive provides a simple, declarative way to add ForesightJS prefetching to your Vue applications. Unlike the [`useForesight` composable](./composable.md), the directive offers a cleaner API with less setup code, but doesn't provide access to [`registerResults`](../configuration/element-settings.md#registration-return-value) or the element's `templateRef`. It's ideal for most use cases where you don't need these advanced features.
...
For more control, you can pass an options object with any [Element Settings](../configuration/element-settings.md).
