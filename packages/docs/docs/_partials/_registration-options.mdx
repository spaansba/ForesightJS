...
The `register()` method returns the full `ForesightElementState` snapshot of the element at registration time, plus five functions: `unregister()`, `subscribe(listener)`, `getSnapshot()`, `subscribeToBounds(listener)`, and `getBounds()`. When a single element is passed, it returns a `ForesightRegisterResult`. When a `NodeList` is passed, it returns `ForesightRegisterResult[]`.
...
Subscribes to logical state changes for this element. The listener is called every time the state snapshot is replaced - never for geometry-only changes during scroll/resize, which live on the separate bounds channel (`subscribeToBounds`). Returns an unsubscribe function. Together with `getSnapshot` this makes the state usable as an external store (e.g. React's `useSyncExternalStore`) without any scroll-induced re-renders.

#### `getSnapshot()`

Returns the current immutable `ForesightElementState` snapshot for this element. The snapshot reference is replaced (never mutated) on every logical change, so the properties spread on the return value itself are only current as of registration; use `getSnapshot()` (optionally with `subscribe`) for the up-to-date state.

#### `subscribeToBounds(listener)`

Subscribes to geometry changes for this element. The listener is called every time the element's position or size changes, which is every scroll/resize tick while the element is visible. Returns an unsubscribe function.

#### `getBounds()`

Returns the current immutable `ElementBounds` snapshot for this element: its measured `originalRect` and the hitSlop-expanded `expandedRect` used for interaction detection.
...
| `hitSlop`                    | `Exclude<HitSlop, number>`          | The normalized hit slop applied to this element, set via the [`hitSlop`](#hitslop) option. The element's rects are not part of the state, read them via `getBounds()`.                                                                                                  |
