...
export default class JavaScriptPage {
  constructor() {
    console.log("JavaScriptPage: Constructor called");
  ...
  render() {
    ...
    subtitle.textContent = 'Move your mouse around the elements below to see the ForesightManager in action';
  ...
  createControls() {
    ...
    debugToggle.className = 'px-4 py-2 font-medium rounded text-white bg-red-500 hover:bg-red-600 mb-3';
    
    debugToggle.addEventListener('click', () => {
      this.debugEnabled = !this.debugEnabled;
      ForesightManager.initialize({ debug: this.debugEnabled });
      
      debugToggle.textContent = this.debugEnabled 
        ? 'Disable Debug Mode' 
        : 'Enable Debug Mode';
      ...
      console.log(`JavaScriptPage: Debug mode ${this.debugEnabled ? 'enabled' : 'disabled'}`);
    });
  ...
  createDemoSection() {
    ...
    const element2 = this.createBox('bg-blue-500', 'Element 2', 'Custom HitSlop (20 top/bottom, 100 left/right)');
    ...
    const unregister3 = this.manager.register(
      element3,
      () => {
  ...
  createPerformanceSection() {
    ...
    perfDesc.textContent = 'Generate multiple elements with 5px hitSlop to test ForesightManager performance';
    perfSection.appendChild(perfDesc);
    
    const generateButton = document.createElement('button');
    generateButton.textContent = 'Generate 50 Elements';
    generateButton.className = 'px-4 py-2 font-medium rounded text-white bg-purple-600 hover:bg-purple-700';
    ...
    generateButton.addEventListener('click', () => {
      ...
      resultsDisplay.textContent = `Generated and registered 50 elements in ${duration.toFixed(2)}ms`;
    });
   ...
   */
  initialize() {
    console.log("JavaScriptPage: Initialize method called");
    
    if (this.initialized) {
      console.log("JavaScriptPage: Already initialized, skipping");
      return;
    }
    ...
    if (pageContainer) {
      console.log("JavaScriptPage: Found container, appending content");
      
      // Clear container first
      if (pageContainer.firstChild) {
        console.log("JavaScriptPage: Clearing existing content");
        pageContainer.innerHTML = '';
      }
      
      pageContainer.appendChild(this.container);
      this.initialized = true;
      console.log("JavaScriptPage: Successfully initialized and appended to DOM");
    } else {
   ...
   */
  cleanup() {
    console.log("JavaScriptPage: Cleanup method called");
    ...
    this.initialized = false;
    console.log("JavaScriptPage: Cleanup complete");
  }
}
